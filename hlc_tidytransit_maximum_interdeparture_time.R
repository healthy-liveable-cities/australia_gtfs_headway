...
gpkg_out <- paste0(Sys.Date(),'_tidy_transit_maximum_interdeparture_2019.gpkg')

analysis_start = '2019-10-08'
analysis_end = '2019-12-05'
...
stop_pct_no_more_than_30_min_wait <- stops_percent_days_no_more_than_30_max_gap %>%
  group_by(stop_id) %>%
  summarise(pct_no_more_than_30_min_wait = max(percent_no_more_than_30_max))

# create spatial features for all stops
stops_sf <- stops_as_sf(gtfs$stops %>% filter(stop_id %in% (stop_pct_no_more_than_30_min_wait %>% pull(stop_id)))) %>%
...
all_stops_current <- stops_sf %>% 
  right_join(stop_pct_no_more_than_30_min_wait, by="stop_id") %>% 
  select(c(stop_id,mode,state,authority,publication_date,pct_no_more_than_30_min_wait,geometry))
st_write(all_stops_current, dsn=gpkg_out, layer="stop_pct_no_more_than_30_min_wait",  layer_options = "OVERWRITE=YES" )
